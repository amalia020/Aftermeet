 ...
 * not in the provider wrapper), calls claudeComplete, parses JSON, and falls
 ...
 * Claude provider is unavailable or its output cannot be parsed. Never throws.
 */
export async function generateDraft(input: {
  ...
  try {
    const outcome = await claudeComplete({
    ...
    const parsed = safeJsonParse<DraftJson>(outcome.data);
